'use strict';
/**
 * XMoment — moment-coin.js  (P0 core: spec → Clanker partner-deploy descriptor → app-level time-box)
 * =====================================================================================================
...
const SPLIT = { interface: 40, creator: 40, clanker: 20 }; // of TOTAL fees: creator 5000 + XMoment 5000 bps (=50% of the 80% pool each) → 40/40, Clanker 20
...
function buildMomentCoin(moment, opts = {}) {
  ...
  if (!isAddr(opts.interfaceFeeRecipient)) throw new Error('opts.interfaceFeeRecipient must be a 0x address (XMoment, the 40% interface slot)');
...
function clankerDeployDescriptor(spec, ctx = {}) {
  ...
  const recipients = [
    ...
    { recipient: spec.fees.interfaceRecipient, admin: spec.fees.interfaceRecipient, bps: 5000, token: 'Both' }, // recipients[1] = XMoment interface slot → 40% of total
  ];
  ...
  return {
    ...
    params: {
      ...
      context: { interface: 'XMoment', platform: ctx.platform || 'farcaster', messageId: spec.momentRef || ctx.messageId || '', id: ctx.id || '' }, // clanker.world provenance only — NOT fee routing
...
if (require.main === module) {
  ...
  const PLATFORM = '0x' + 'cd'.repeat(20);            // XMoment's 40% interface slot
  ...
  const checks = [
    ...
    ['descriptor carries the grounded fees (static 100bps=1%) + context (interface XMoment, momentRef bound) from clanker.gitbook.io', desc.params.fees.type === 'static' && desc.params.fees.clankerFee === 100 && desc.params.fees.pairedFee === 100 && desc.params.context.interface === 'XMoment' && desc.params.context.messageId === 'wc:fra-bra:71' && /clanker\.gitbook\.io/.test(desc.grounding)],
